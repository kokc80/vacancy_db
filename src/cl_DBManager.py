...
class DBManager(DBClass):
    def get_companies_and_vacancies_count(self, db_name: str):
        ...
        cur = conn.cursor()
        tmp_query = (f"SELECT tv.vac_id,tv.vac_name,te.emp_id,te.emp_name FROM tab_vac as tv, tab_emp as te "
                     f"where tv.emp_id = te.emp_id")
        tmp_query = (f"SELECT tv.emp_id, te.emp_name, count(vac_id) FROM tab_vac as tv "
    ...
    def get_all_vacancies(self, db_name: str):
        ...
        conn.autocommit = True
        cur = conn.cursor()
        tmp_query = (f"select tv.vac_name, te.emp_name, tv.sal_from, tv.sal_to, tv.sal_cur, tv.vac_url "
    ...
    def get_vacancies_with_higher_salary(self, db_name: str):
        ...
        conn.autocommit = True
        cur = conn.cursor()
        tmp_query = (f"select tv.vac_name, tv.sal_from, tv.sal_to, tv.sal_cur, tv.vac_url from tab_vac as tv "
    ...
    def get_vacancies_with_keyword(self, db_name: str, keyword: str):
        ...
        conn.autocommit = True
        cur = conn.cursor()
        tmp_query = (f"select tv.vac_name, tv.sal_from, tv.sal_to, tv.sal_cur, tv.vac_url "
